            ...
            <li className="item-img-slider__wrap" style={(device)? {maxWidth: '12em', minWidth: '11em'} : {minWidth: '20em'}}>
                <div className="wrapper">
                    ...
                    <div className="slider">
                        {indexes.map((i) => (<div key={item?.article + `${i}`} className={"slides slide" + `${i}`}><img src={'https://levalexeev.github.io/' + item?.previewImages[i-1]}/></div>))}
                    </div>
                    <div className="controls">
                        {indexes.map((i) => (<label key={item?.article + `${i}`} htmlFor={"slide" + `${i}`} style={(device)? {width: '40px', height: '40px'} : {width: '60px', height: '60px'}} onClick={() => setActiveImg(i)}><img src={'https://levalexeev.github.io/' + item?.previewImages[i-1]}/></label>))}
                    </div>
                </div>
